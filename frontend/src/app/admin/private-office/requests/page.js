...
export default function RequestList() {
  ...
  const handleApprove = async (request) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Handle conflict error (room already occupied)
        const message = response.message || 'Failed to approve request. Please try again.';
        const title = response.error === 'Conflict' ? 'Room Already Occupied' : 'Error';
      ...
      const message = error.response?.data?.message || error.message || 'Failed to approve request. Please try again.';
      const title = error.response?.data?.error === 'Conflict' ? 'Room Already Occupied' : 'Error';
  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-5 lg:gap-6 animate-fadeIn">
      ...
      {/* Error Modal - Using Portal to cover entire page */}
      {showErrorModal && mounted && createPortal(
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[9999] p-4" onClick={() => setShowErrorModal(false)}>
          <div className="bg-white rounded-2xl p-6 sm:p-8 flex flex-col items-center gap-4 max-w-md w-full animate-[slideUp_0.3s_ease] shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center">
              <svg className="w-8 h-8 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </div>
            <h3 className="text-xl font-bold text-slate-800">{errorTitle}</h3>
            <p className="text-gray-600 text-center text-sm leading-relaxed">{errorMessage}</p>
            <button
              onClick={() => setShowErrorModal(false)}
              className="mt-2 px-6 py-2.5 bg-red-600 text-white rounded-lg font-semibold hover:bg-red-700 transition-colors duration-200 shadow-lg hover:shadow-xl"
            >
              Close
            </button>
          </div>
